...
import {
    setIncludeTransactionsTableStatuses,
    setIncludeTransactionsTableTypes, setTransactionsSortBy,
    setTransactionsSortDirection, setTransactionsPageSize,
...
export default function TransactionsTableControls() {
    const include_transactions_types: { [key: string]: boolean } = useSelector((state: any) => state.data.include_transactions_types);
    const include_transactions_statuses: { [key: string]: boolean } = useSelector((state: any) => state.data.include_transactions_statuses);
    const include_transactions_columns: { [key: string]: boolean } = useSelector((state: any) => state.data.include_transactions_columns);
    const transactions_table_sort: string = useSelector((state: any) => state.data.transactions_sort_by);
    const transactions_table_sort_direction: string = useSelector((state: any) => state.data.transactions_sort_direction);
    const transactions_table_page_size: number = useSelector((state: any) => state.data.transactions_page_size);
    ...
    const handleIncludeTransactionsTypes = (key: string) => {
        const updatedFilters = {
            ...
            [key]: !include_transactions_types[key]
        };
    ...
    const handleIncludeTransactionsStatuses = (key: string) => {
        const updatedFilters = {
            ...
            [key]: !include_transactions_statuses[key]
        };
        dispatch(setIncludeTransactionsTableStatuses(updatedFilters));
    };

    // const handleIncludeTransactionsColumns = (key: string) => {
    //     const updatedFilters = {
    //         ...include_Transactions_columns,
    //         [key]: !include_Transactions_columns[key]
    //     };
    //     dispatch(setIncludeTransactionsTableColumns(updatedFilters));
    // }

    const handleIncludeAllTypes = () => {
        setIncludeAllTypes(!includeAllTypes)
        const updatedFilters = Object.fromEntries(Object.keys(include_transactions_types).map(key => [key, !includeAllTypes]))
        dispatch(setIncludeTransactionsTableTypes(updatedFilters));
    };

    const handleIncludeAllStatuses = () => {
        setIncludeAllStatuses(!includeAllStatuses)
        const updatedFilters = Object.fromEntries(Object.keys(include_transactions_statuses).map(key => [key, !includeAllStatuses]))
        dispatch(setIncludeTransactionsTableStatuses(updatedFilters));
    };

    // const handleIncludeAllColumns = () => {
    //     setIncludeAllColumns(!includeAllColumns)
    //     const updatedFilters = Object.fromEntries(Object.keys(include_Transactions_columns).map(key => [key, !includeAllColumns]))
    //     dispatch(setIncludeTransactionsTableColumns(updatedFilters));
    // };
    ...
    return (
        <div className="">
            <div className="flex items-center">
                <Input
                    placeholder="Search by property or participants"
                    value={search}
            ...
            <div className="flex items-center py-4">
                ...
                {/* <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        <ScrollArea style={{ height: `calc(100vh - 500px)`, minHeight: '400px' }}>
                            ...
                            {Object.entries(include_Transactions_columns).map(([key, value]) => (
                                <DropdownMenuCheckboxItem
                                    key={key}
                                    className="capitalize"
                                    checked={value}
                                    onCheckedChange={() => handleIncludeTransactionsColumns(key)}
                                >
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        ...
                        {Object.entries(include_transactions_types).map(([key, value]) => (
                            <DropdownMenuCheckboxItem
                                key={key}
                                className="capitalize"
                                checked={value}
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        ...
                        {Object.entries(include_transactions_statuses).map(([key, value]) => (
                            <DropdownMenuCheckboxItem
                                key={key}
                                className="capitalize"
                                checked={value}
