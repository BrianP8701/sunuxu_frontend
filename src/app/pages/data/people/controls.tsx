...
import { setIncludePeopleTableStatuses, setIncludePeopleTableTypes, setPeopleSortBy, setPeopleSortDirection, setPeoplePageSize } from '@/app/store/appSlice';
...
export default function PeopleTableControls() {
    const include_people_table_types: { [key: string]: boolean } = useSelector((state: any) => state.app.include_people_types)
    const include_people_table_statuses: { [key: string]: boolean } = useSelector((state: any) => state.app.include_people_statuses)
    const people_table_sort: string = useSelector((state: any) => state.app.people_sort_by)
    const people_table_sort_direction: string = useSelector((state: any) => state.app.people_sort_direction)
    const people_table_page_size: number = useSelector((state: any) => state.app.people_page_size)
    ...
    const handleIncludePeopleTypes = (key: string) => {
        if (include_people_table_types[key]) {
            setIncludeAllTypes(false)
        }
        const updatedFilters = {
            ...include_people_table_types,
            [key]: !include_people_table_types[key]
        };
        dispatch(setIncludePeopleTableTypes(updatedFilters));
    }

    const handleIncludePeopleStatuses = (key: string) => {
        if (include_people_table_statuses[key]) {
            setIncludeAllStatuses(false)
        }
        const updatedFilters = {
            ...include_people_table_statuses,
            [key]: !include_people_table_statuses[key]
        };
    ...
    const handleIncludeAllTypes = () => {
        ...
        const updatedFilters = Object.fromEntries(Object.keys(include_people_table_types).map(key => [key, !includeAllTypes]))
        dispatch(setIncludePeopleTableTypes(updatedFilters));
    }

    const handleIncludeAllStatuses = () => {
        setIncludeAllStatuses(!includeAllStatuses)
        const updatedFilters = Object.fromEntries(Object.keys(include_people_table_statuses).map(key => [key, !includeAllStatuses]))
        dispatch(setIncludePeopleTableStatuses(updatedFilters));
    }
        ...
        <div className="flex items-center py-4">
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    ...
                    {Object.entries(include_people_table_types).map(([key, value]) => (
                        <DropdownMenuCheckboxItem
                            key={key}
                            className="capitalize"
                            checked={value}
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    ...
                    {Object.entries(include_people_table_statuses).map(([key, value]) => (
                        <DropdownMenuCheckboxItem
                            key={key}
                            className="capitalize"
                            checked={value}
